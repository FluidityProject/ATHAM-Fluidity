...
!----------------------------------------------------------------------------
! 
! This routine writes a file LWRIN containing a matrix of raddown per gpoint
! vs height
!----------------------------------------------------------------------------
...
! Clouds
      ...
      real(kind=rb), intent(in) :: taucloud(:,:)      ! layer cloud optical depth
                                                      !    Dimensions: (nlayers,nbndlw)
      REAL(kind=rb), intent(in) :: rad_down(:)        ! lw_rad down at model top 
...
! Declarations for cloud overlap adjustment
      ...
      REAL(kind=rb) :: rad_dump(140,nlayers)
      INTEGER(kind=im) :: i, k
! ------- Definitions -------
! input
...
! Radiative transfer starts here.
         radld   = 0._rb ! rad_down(igc)    ! inputing radiation at model top
         radclrd = radld          
         iclddn  = 0
...
! Set heating rate to zero in top layer
      ...
      OPEN(10,file='input/LWRIN',form='unformatted', status='new')
      WRITE(10) ((rad_dump(i,k), i=1,140), k=1,nlayers)
      CLOSE(10)
