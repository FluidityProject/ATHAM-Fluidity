...
module advection_diffusion_cg
  ...
  use equation_of_state, only : scale_pressure
...
contains
  ...
  subroutine assemble_advection_diffusion_element_cg(ele, t, matrix, rhs, &
    ...
    type(scalar_field), intent(in) :: pressure, pressure_scale
    ...
    ! Source
    if(have_source .and. (.not. add_src_directly_to_rhs)) & 
      ...
      case(FIELD_EQUATION_KEPSILON)
        ...
        if(abs(1.0 - beta) > epsilon(0.0)) then
          ...
          advection_mat = advection_mat &
                    - (1.0-beta) * shape_shape(test_function, t_shape, (velocity_div_at_quad*density_at_quad + udotgradrho_at_quad)* detwei)
        end if
      case default
  ...
  subroutine add_pressurediv_element_cg(ele, test_function, t, velocity, pressure, pressure_scale, nvfrac, du_t, detwei, rhs_addto)
    ...
    if (compressible) &
    rhs_addto = rhs_addto - shape_rhs(test_function, ele_div_at_quad(velocity, ele, du_t) * &
        ele_val_at_quad(pressure_scale, ele)*ele_val_at_quad(pressure, ele) * detwei)
